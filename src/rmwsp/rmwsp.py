...
def main():
    ...
    parser = argparse.ArgumentParser(description="人民网-习重要讲话视频采集器")
    ...
    if mode == 0:
        adapter.info(f"人民网-习重要讲话视频信息采集-增量模式-启动")
    if mode == 1:
        adapter.info(f"人民网-习重要讲话视频信息采集-全量模式-启动")
    ...
    browser = Chromium(co)
    # browser = Chromium()
    main_url = ["http://cpc.people.com.cn/shipin/"]
    list_count = -1
    for each_url in main_url:
        duplicate_time = 0  # 连续的重复记录数
        list_count += 1
        if list_count == 0:
            adapter.info(f"列表1-人民网-习重要讲话视频开始爬取")
        ...
        #获取最大页码
        max_page_num = int(list_tab.ele(".page").children()[-2].text)
        page_next = 1
        ...
        while page_next <= max_page_num:
            ...
            #首先是进入正确的页码判断
            while True:
                real_page_now = int(list_tab.ele(".common_current_page").text)
                if real_page_now == page_next:
                    adapter.info(f"进入了正确的页码：{page_next}")
                    break
                adapter.info(f"调整进入正确页码中...")
                if page_next == 1:
                    list_tab.get("http://cpc.people.com.cn/shipin/")
                else:
                    list_tab.get(f"http://cpc.people.com.cn/shipin/index{page_next}.html")
                time.sleep(1)


            #这里进入了正确的页码，page_next相当于page_now
            uls_all_video = list_tab.ele(".fl").children("@tag()=ul")
            list_all_video_li = []
            for ul_each_video in uls_all_video:
                hrefs_all_video_for_one_ul = ul_each_video.eles("t=a").get.attrs("href")
                list_all_video_li.extend(hrefs_all_video_for_one_ul)
            ...
            for href_each_video in list_all_video_li:
                ...
                #分类
                one_element.fen_lei = video_tab.ele(".fl").text.strip()
