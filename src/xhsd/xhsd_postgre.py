...
def is_duplicate(str_id):
    ...
    try:
        cursor.execute(f"SELECT * FROM rs_correct_resources WHERE str_id='{str_id}'")
...
def update_price(str_id, price):
    ...
    if rows:
        ...
        if price_before != price:
            ...
            cursor = connection.cursor()
            cursor.execute(f"""UPDATE rs_correct_resources SET price='{price}', update_time='{datetime.now()}'  WHERE str_id='{str_id}'""")
